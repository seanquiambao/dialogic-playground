...
var _first_speaker: DialogicCharacter
...
func find_first_speaker() -> void:
	...
	for event in current_timeline.events:
		if not event is DialogicTextEvent:
			continue
		if event.character.description.contains("protagonist"):
			continue
		_first_speaker = event.character
		break
...
func register_character(character: DialogicCharacter, scene_character: Node3D) -> void:
	...
	if not _first_speaker:
		find_first_speaker()

	if _first_speaker.get_character_name() == character_name:
...
func _on_speaker_updated(character: DialogicCharacter):
	if not character:
		return
	if character.description.contains("protagonist"):
		return
